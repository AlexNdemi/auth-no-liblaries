...
export async function updateUserSessionData(
  ...
  await redisClient.set(`session:${sessionId}`, JSON.stringify(parsedUser),  "EX", SESSION_EXPIRATION_SECONDS,
  )
}
...
export async function createUserSession(
  ...
  const parsedUser = sessionSchema.parse(user)
  await redisClient.set(`session:${sessionId}`, JSON.stringify(parsedUser),
